...
CreateThread(function()
    ...
    function FM.FreeCam:Spawner(where, heading, type)
        ...
        FM.Painter:DrawText("~w~[~y~Tiago Menu~w~] " .. (name or "?"), 4, false, FM:ScrW() - 360 - 21 - offX, 21, 0.35, 255, 255, 255, 255)
        local sY = 30
        local max_opts = 30
    ...
    function FM.FreeCam:DrawMode()
        ...
        FM:ScreenText("~w~[~y~Tiago Menu~w~] Freecam Mode: ~y~" .. name, 4, 1.0, 0.5, 0.97, 0.35, 255, 255, 255, 225)
    end
    ...
    function FM:DrawNotifications()
        ...
        self.Painter:DrawText("~w~[~y~Tiago Menu~w~] Notifications", 4, false, n_x, n_y, 0.35, 193, 0, 19, dict.math.ceil(notif_alpha + 55))

        if #self.Notifications <= 0 then
            self.Painter:DrawText("~w~No new notifications to display.", 4, false, n_x + 0.5, n_y + 33, 0.35, 193, 0, 19, dict.math.ceil(notif_alpha + 55))
        else
            local notifY = n_y + 33
            local s_y = notifY
            local id = 1
