...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', '~w~BRUTAN V7')
	WarMenu.SetSubTitle('MainMenu', 'Welcome to BRUTAN Cheats '..player..'.')
...
-- Misc
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			drawNotification("Hello ~g~" .. player .. " ~s~! Ver ~r~V7~s~ ∑")
			drawNotification("You are using BRUTAN Cheats Menu~n~Private ! ~p~Enjoy :3")
			...
			elseif WarMenu.Button('~r~Unload BRUTAN Menu') then
				Enabled = false
			elseif WarMenu.Button("Made by BRUTAN#7799 :3 ") then
			end
...
-- Credit Menu			
		...
		elseif WarMenu.IsMenuOpened("CreditMenu") then
			if WarMenu.Button("Unknown ~b~>~s~ 5$ Donation !") then
			end
