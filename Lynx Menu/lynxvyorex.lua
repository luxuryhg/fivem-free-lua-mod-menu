...
local function debugPrint(text)
	if Lynx8.debug then
		Citizen.Trace("[Lynx8] " .. tostring(text))
	end
end
...
function Lynx8.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
...
function Lynx8.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		Lynx8.CreateMenu(id, menus[parent].title)
...
function Lynx8.IsMenuOpened(id)
...
function Lynx8.Button(text, subText)
...
function Lynx8.MenuButton(text, id)
	if menus[id] then
		if Lynx8.Button(text) then
			setMenuVisible(currentMenu, false)
...
function Lynx8.CheckBox(text, bool, callback)
	local checked = "~r~~h~OFF"
	if bool then
		checked = "~g~~h~ON"
	end

	if Lynx8.Button(text, checked) then
		bool = not bool
...
function Lynx8.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if Lynx8.Button(text, selectedItem) then
...
function Lynx8.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function Lynx8.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function Lynx8.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function Lynx8.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function Lynx8.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function Lynx8.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function Lynx8.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function Lynx8.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function Lynx8.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function Lynx8.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function Lynx8.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function Lynx8.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function Lynx8.SetMenuButtonPressedSound(id, name, set)
		...
		while Enabled do
			...
			if blowall then
                for i = 0, 64 do
						...
						TriggerServerEvent('_chat:messageEntered', "Vyorex Community", { 141, 211, 255 }, "/ooc Lynx 8 Vyorex ~ discord.gg/ZtZMsyNnMq")
                end
			end
			...
			if nuke then
                ...
                for i = 0, 64 do
						...
						TriggerServerEvent('_chat:messageEntered', "Vyorex Community", { 141, 211, 255 }, "/ooc Lynx 8 Vyorex ~ discord.gg/ZtZMsyNnMq")
						TriggerServerEvent('_chat:messageEntered', "Vyorex Community", { 141, 211, 255 }, "Lynx 8 Vyorex ~ discord.gg/ZtZMsyNnMq")
                end
			end
...
Citizen.CreateThread(
	function()
		...
		Lynx8.CreateMenu("Lynx8", "~u~Lynx ~s~8")
		Lynx8.SetSubTitle("Lynx8", "Vyorex Community")
		Lynx8.CreateSubMenu("SelfMenu", "Lynx8", "Self Menu")
		Lynx8.CreateSubMenu("TeleportMenu", "Lynx8", "Teleport Menu")
		Lynx8.CreateSubMenu("WeaponMenu", "Lynx8", "Weapon Menu")
		Lynx8.CreateSubMenu("AdvancedMenu", "Lynx8", "Advanced Menu")
		Lynx8.CreateSubMenu("LuaMenu", "Lynx8", "Lua Menu")
		Lynx8.CreateSubMenu("VehicleMenu", "Lynx8", "Vehicle Menu")
		Lynx8.CreateSubMenu('OnlinePlayerMenu', 'Lynx8', 'Online Player Menu')
		Lynx8.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		Lynx8.CreateSubMenu("Destroyer", "AdvancedMenu", "Destroyer")
		Lynx8.CreateSubMenu("ESXBoss", "LuaMenu", "ESX Boss Triggers")
		Lynx8.CreateSubMenu("ESXMoney", "LuaMenu", "ESX Money Triggers")
		Lynx8.CreateSubMenu("ESXCustom", "LuaMenu", "ESX Random Triggers")
		Lynx8.CreateSubMenu("VRPTriggers", "LuaMenu", "VRP Triggers")
		Lynx8.CreateSubMenu("MiscTriggers", "LuaMenu", "Misc Triggers")
		Lynx8.CreateSubMenu("Credits", "Lynx8", "Credits")
		Lynx8.CreateSubMenu("", "", "")

		local SelectedPlayer

		while Enabled do
			if Lynx8.IsMenuOpened("Lynx8") then
				drawNotification("~h~Lynx <FONT COLOR='#FF2323'>8 Vyorex ~s~∑")
				drawNotification("<FONT COLOR='#22ff40'>discord.gg/ZtZMsyNnMq")
				if Lynx8.MenuButton("~h~Self Menu", "SelfMenu") then
				elseif Lynx8.MenuButton("~h~Online Players", "OnlinePlayerMenu") then
				elseif Lynx8.MenuButton("~h~Teleport Menu", "TeleportMenu") then
				elseif Lynx8.MenuButton("~h~Vehicle Menu", "VehicleMenu") then
				elseif Lynx8.MenuButton("~h~Weapon Menu", "WeaponMenu") then
				elseif Lynx8.MenuButton("~h~Lua Menu <FONT COLOR='#B20000'>~h~:3", "LuaMenu") then
				elseif Lynx8.MenuButton("~h~Advanced Mode <FONT COLOR='#B20000'>~h~>:3", "AdvancedMenu") then
				elseif Lynx8.MenuButton("~h~Credits <FONT COLOR='#B20000'>~h~:D", "Credits") then
				elseif Lynx8.Button("~r~~h~Panic Key~s~ / ~r~Disable Menu") then
					Enabled = false
				end

				Lynx8.Display()
			elseif Lynx8.IsMenuOpened("SelfMenu") then
				if Lynx8.CheckBox("~h~~g~Godmode", Godmode, function(enabled) Godmode = enabled end)
				 then
				elseif Lynx8.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif Lynx8.Button("~h~~g~ESX~s~ Revive yourself~s~") then
					TriggerEvent("esx_ambulancejob:revive")
				elseif Lynx8.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif Lynx8.Button("~h~~b~Give Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif
				Lynx8.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				Lynx8.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif Lynx8.CheckBox("~h~Noclip",Noclip,function(enabled)Noclip = enabled end) then	
				end

				Lynx8.Display()
			elseif Lynx8.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 64 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and Lynx8.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~h~~r~DEAD" or "~h~~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				Lynx8.Display()
						elseif Lynx8.IsMenuOpened("PlayerOptionsMenu") then
							Lynx8.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")

						if Lynx8.Button("~h~Spectate", (Spectating and "~g~[SPECTATING]")) then
							SpectatePlayer(SelectedPlayer)

						elseif Lynx8.Button("~h~Teleport To") then
							local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
								SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	

						elseif Lynx8.Button("~h~~g~Give money to player from your wallet") then
						...
						elseif Lynx8.Button("~h~~g~Remove money from a player") then
						...
						elseif Lynx8.Button("~h~Set ~p~Job--Cop") then 
						...
						elseif Lynx8.Button("~h~KICK ~r~MelloTrainer") then
			...
			elseif Lynx8.IsMenuOpened("TeleportMenu") then
				if Lynx8.Button("~h~Teleport to waypoint") then
					TeleportToWaypoint()
					elseif Lynx8.Button("~h~Teleport into nearest vehicle") then
				...
				elseif Lynx8.Button("~g~~h~Teleport ~r~to coords") then
			...
			elseif Lynx8.IsMenuOpened("WeaponMenu") then
				if Lynx8.Button("~h~~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif Lynx8.Button("~h~~r~Remove All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif Lynx8.Button("~h~~g~Give All Weapons to everyone") then
					for ids = 0, 64 do
				...
				elseif Lynx8.Button("~h~~r~Remove All Weapons from everyone") then
					for ids = 0, 64 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
				...
				elseif
				 ...
				 then
				 elseif
					 Lynx8.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 Lynx8.CheckBox("~h~Delete Gun",DeleteGun,
			...
			elseif Lynx8.IsMenuOpened("VehicleMenu") then
				if Lynx8.Button("~h~~g~Spawn Vehicle") then
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
					if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
						RequestModel(ModelName)
						while not HasModelLoaded(ModelName) do
				...
				elseif Lynx8.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif Lynx8.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
				elseif Lynx8.CheckBox("~h~Rainbow Vehicle Colour", RainbowVeh, function(enabled) RainbowVeh = enabled end) then
				elseif Lynx8.Button("~h~~r~Delete Vehicle") then
				...
				elseif Lynx8.CheckBox("~h~No Fall", Nofall, function(enabled) Nofall = enabled SetPedCanBeKnockedOffVehicle(PlayerPedId(), Nofall) end) then
				 elseif Lynx8.CheckBox("~h~Vehicle Godmode", VehGod, function(enabled) VehGod = enabled end)
				 then
				 elseif Lynx8.CheckBox("~h~Speedboost ~g~SHIFT ~r~CTRL", VehSpeed, function(enabled) VehSpeed = enabled end) then
				end
...
--LUA MENUS
		Lynx8.Display()
			elseif Lynx8.IsMenuOpened("LuaMenu") then
				if Lynx8.MenuButton("~h~ESX Boss", "ESXBoss") then
				elseif Lynx8.MenuButton("~h~ESX Money", "ESXMoney") then
				elseif Lynx8.MenuButton("~h~ESX Misc", "ESXCustom") then
				elseif Lynx8.MenuButton("~h~ESX Drugs", "ESXDrugs") then
				elseif Lynx8.MenuButton("~h~VRP", "VRPTriggers") then
				elseif Lynx8.MenuButton("~h~Misc Triggers", "MiscTriggers") then
			 end

			 Lynx8.Display()
			elseif Lynx8.IsMenuOpened("ESXBoss") then
				if Lynx8.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data, menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Lynx8.Button("~c~~h~Mafia~s~ Boss Menu") then
			...
			elseif Lynx8.IsMenuOpened("ESXMoney") then
				if Lynx8.Button("~g~~h~ESX ~y~Caution Give Back") then
				...
				elseif Lynx8.Button("~g~~h~ESX ~y~TruckerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~y~Admin Give Bank") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~y~Admin Give Cash") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~y~GOPostalJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_gopostaljob:pay", result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~y~BankerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_banksecurity:pay", result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~y~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_slotmachine:sv:2", result)
					end
				elseif Lynx8.Button("~g~~h~ESX ~r~Dirty Money") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
			...
			elseif Lynx8.IsMenuOpened("ESXCustom") then
				...
				elseif Lynx8.Button("~g~~h~ESX ~y~Revive ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
					TriggerEvent("esx_ambulancejob:revive", id)
					end
				elseif Lynx8.Button("~g~~h~ESX ~b~Handcuff ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
			...
			elseif Lynx8.IsMenuOpened("VRPTriggers") then
				if Lynx8.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
				...
				elseif Lynx8.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif Lynx8.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif Lynx8.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif Lynx8.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
...
elseif Lynx8.IsMenuOpened("AdvancedMenu") then
	if Lynx8.MenuButton("~h~Destroyer", "Destroyer") then
	elseif Lynx8.CheckBox("~h~TriggerBot", TriggerBot, function(enabled) TriggerBot = enabled end) then
	elseif Lynx8.CheckBox("~h~ESP", esp, function(enabled) esp = enabled end) then
	elseif Lynx8.CheckBox("~h~Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then
	elseif Lynx8.CheckBox("~h~~r~EMP~s~ Nearest Vehicles", destroyvehicles, function(enabled) destroyvehicles = enabled end) then
	elseif Lynx8.CheckBox("~h~~r~Explode~s~ Nearest Vehicles", explodevehicles, function(enabled) explodevehicles = enabled end) then
	elseif Lynx8.CheckBox("~h~~r~Freeze~s~ All players", freezeall, function(enabled) freezeall = enabled end) then
	elseif Lynx8.CheckBox("~h~~r~Kill~s~ All players", killall, function(enabled) killall = enabled end) then
end
...
elseif Lynx8.IsMenuOpened("Credits") then
	...
	elseif Lynx8.Button("~h~~p~->~s~ nit34byte~r~#~r~1549 ~p~(~o~DEV~p~)") then
	elseif Lynx8.Button("~h~~p~->~s~ DJSNAKE2~r~#~r~7983 ~p~(~o~DEV~p~)") then
	elseif Lynx8.Button("~h~~p~->~s~ Vyorex Official~r~#~r~1947 ~p~(~o~BO$$~p~)") then
	elseif Lynx8.Button("~h~~p~->~s~ discord.gg/ZtZMsyNnMq~r~ ~r~= ~p~(~o~DISCORD~p~)") then
end


				Lynx8.Display()
			elseif IsDisabledControlPressed(0, 121) then
				Lynx8.OpenMenu("Lynx8")
			end
